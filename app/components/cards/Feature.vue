    ...
    <div class="absolute flex flex-col gap-6 z-10" style="left: 35px; top: 34px; width: 341px;">
      ...
      <ul v-if="showBullets !== false" class="flex flex-col gap-3">
        <li v-for="point in points" :key="point" class="flex items-center gap-6">
          ...
          <p class="text-white/80 text-base leading-[1.4] font-schibsted">{{ point }}</p>
        </li>
      </ul>

      <!-- Plain description (no bullets) -->
      <p v-else class="text-white/80 text-base leading-[1.4] font-schibsted whitespace-pre-line">{{ description }}</p>
    </div>
  </div>
</template>
